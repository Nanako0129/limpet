...
/// Mirrors a SAFE, enumerated subset of `LimpetSettings` to
/// `~/.config/limpet/settings.json` so it can be hand-edited live by an
/// external agent or human, the same way `{shortId}.profile.json` mirrors a
/// profile.
///
/// Only the two keys in `SafeKey` are ever written. Adding a new key is an
/// explicit, reviewable opt-in — never derived automatically from
/// `LimpetSettings`' full key set — so any sensitive key can never leak
/// into the file just because someone added a property to `LimpetSettings`.
enum AppSettingsFileStore {
    ...
    /// Snapshot the current safe settings from `LimpetSettings` (the source
    /// of truth). `isLoginItemEnabled` is passed in because that state lives
    /// in `SMAppService`, not `LimpetSettings`.
    static func currentSafeSettings(isLoginItemEnabled: Bool) -> [SafeKey: Bool] {
        [
            .debugLoggingEnabled: LimpetSettings.debugLoggingEnabled,
