...
final class ProfileStore: ObservableObject {
    ...
    /// self-test isolation; defaults to the real `~/.config/limpet/profiles`.
    ...
    /// callable off `@MainActor` so the headless CLI (`LimpetCLI`) can read
    ...
    /// (`LimpetCLI`'s `profile create`/`enable`/`disable`) writes the exact
