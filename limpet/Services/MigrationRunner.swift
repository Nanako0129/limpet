...
enum MigrationRunner {
    private static let schemaVersionKey = "limpet.schemaVersion"
    ...
    static func runPendingMigrations() {
        ...
        for migration in migrations where migration.version > currentVersion {
            print("[limpet] Running migration v\(migration.version): \(migration.description)")
            ...
            do {
                ...
                print("[limpet] Migration v\(migration.version) complete")
            } catch {
                print("[limpet] Migration v\(migration.version) failed: \(error.localizedDescription)")
...
/// This subsumes the logic previously in LimpetApp.performMigrationIfNeeded().
struct MigrationV1LegacyToMultiProfile: ProfileMigration {
    ...
    func migrateUserDefaults(_ defaults: UserDefaults) throws {
        ...
        do {
            ...
            print("[limpet] Warning: failed to install migrated profile: \(error)")
